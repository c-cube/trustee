...
module Make() = struct
  ...
  let rules_ac =
    Conv.combine_l [
      Rw.AC_rule.make ctx ~f:(const c_f2 []) ~assoc:ax_f2_assoc ~comm:ax_f2_comm ()
      |> Rw.AC_rule.to_conv;
      Rw.AC_rule.make ctx ~f:(const c_g2 []) ~assoc:ax_g2_assoc ~comm:ax_g2_comm ()
      |> Rw.AC_rule.to_conv;
    ]
  ...
  let assert_e_eq t1 t2 =
    assert_equal ~cmp:E.equal ~printer:E.to_string t1 t2
  ...
  let gen_e : E.t QCheck.Gen.t =
    ...
    let gen_rec self depth =
      ...
      if depth = 0 then frequency atom_cases
      else frequency (atom_cases @ [
          (2, let+ x=self (depth-1) in f1 x);
          (2, let+ x=self (depth-1) in g1 x);
          (1, let+ x=self (depth-1) and+ y = self (depth-1) in f2 x y);
          (1, let+ x=self (depth-1) and+ y = self (depth-1) in g2 x y);
          (1, let+ x=oneofl [v_x; v_y; v_z]
  ...
  let arb_e =
    QCheck.make ~print:E.to_string ~shrink:shrink_e gen_e
end
...
let t_rw_idempotent1 : test =
  ...
  QCheck.Test.make ~name:"rw_idempotent1" arb_e @@ fun e ->
  let rhs, _ = Conv.apply_e rules_ac ctx e in
  let rh2, _ = Conv.apply_e rules_ac ctx rhs in
  assert_e_eq rhs rh2; true

let t_rw_idempotent2 : test =
  let module M = Make() in let open M in
  QCheck_ounit.to_ounit2_test ?rand:None @@
  QCheck.Test.make ~name:"rw_idempotent2" arb_e @@ fun e ->
