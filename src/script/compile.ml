...
module Env = struct
  ...
  let add_stanza self (st : VM.Stanza.t) : t =
    ...
    | VM.Stanza.Define_thunk { name; value } -> add_thunk name value self
    | VM.Stanza.Define_chunk { name; value } -> add_chunk name value self
    | VM.Stanza.Eval _ -> self
    | VM.Stanza.Declare _ | VM.Stanza.Define _ | VM.Stanza.Prove _ -> self
  (* TODO *)
  (*
  ...
  let pp out self =
    ...
    Fmt.fprintf out "@[<2>env{@ %a@;<1 -2>}@]" (pp_iter pp_kv)
      (M.to_iter self.vals)
end
...
module Compile_ = struct
  ...
  let create ?fun_name env ~n : st =
    {
      locals = Str_map.empty;
      env;
      fun_name;
      cb = CB.create ~n_args:n ~n_ret:1 ();
      n_regs = n;
      free_regs = [];
    }
  ...
  let recycle_reg self n = self.free_regs <- n :: self.free_regs
  ...
  let rec compile_expr (self : st) (e : A.expr) : unit =
    ...
    | A.E_binop (op, a, b) ->
      ...
      | A.Plus ->
        default_args ();
        CB.push_i self.cb I.add
      | A.Minus ->
        default_args ();
        CB.push_i self.cb I.sub
      | A.Times ->
        default_args ();
        CB.push_i self.cb I.mult
      ...
      | A.Leq ->
        default_args ();
        CB.push_i self.cb I.leq
      | A.Lt ->
        default_args ();
        CB.push_i self.cb I.lt
      | A.Geq ->
        recurse b;
        recurse a;
        CB.push_i self.cb I.leq
    ...
    | A.E_if { test; then_; elseif; else_ } ->
      ...
      | Some bl -> compile_block_return self bl);

      (* jump here from successful branches *)
      let here = CB.cur_pos self.cb in
      List.iter (fun pos -> CB.set_i self.cb pos (I.jmp here)) !branch_ends
    | A.E_block bl ->
    ...
    | A.E_const c ->
      ...
      | A.C_atom _ -> Error.failf ~loc (fun k -> k "TODO: atoms"))
    | A.E_logic _ -> assert false
  ...
  and compile_var (self : st) (s : A.var) : unit =
    ...
    | None ->
      ...
      | None, _ ->
        Error.failf ~loc:s.loc (fun k -> k "Variable %S is not in scope" s.view))
  ...
  and compile_block_items (self : st) (l : A.block_item list) : bool =
    ...
    let clear_stack () = if !returns then CB.push_i self.cb I.drop in
    ...
    let rec loop l =
      ...
      | { With_loc.view = A.Bl_let (v, e); _ } :: tl ->
        ...
        CB.push_i self.cb (I.rstore reg);
        self.locals <-
          Str_map.add v.view { lreg = reg; lvar = false } self.locals;
      ...
      | { With_loc.view = A.Bl_var (v, e); _ } :: tl ->
        ...
        CB.push_i self.cb (I.rstore reg);
        self.locals <-
          Str_map.add v.view { lreg = reg; lvar = true } self.locals;

        loop tl
      | { With_loc.view = A.Bl_assign (v, e); loc } :: tl ->
        clear_stack ();
        (match Str_map.find_opt v.view self.locals with
        | Some { lreg; lvar = true } ->
          compile_expr self e;
          CB.push_i self.cb (I.rstore lreg)
        | Some _ ->
          Error.failf ~loc (fun k ->
              k "cannot assign %S, it is not a variable" v.view)
        | None ->
          Error.failf ~loc (fun k ->
              k "cannot assign unknown variable %S" v.view));
      ...
      | { With_loc.view = A.Bl_while (cond, bl); loc } :: tl ->
        ...
        CB.push_i self.cb (I.jmp pos_start);
  ...
  let compile_fn ~env ?fun_name vars body : VM.Chunk.t =
    ...
    if not ret then CB.push_i st.cb I.nil;
    (* make sure we return sth *)
