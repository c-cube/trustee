...
(** Context for the locations.
    ...
    The context provides information that is common to all locations within the
    same file, allowing each location to fit in one integer. *)
...
(** [add_offset ~off loc] is [loc], shifted so that its offset 0 actually starts
    at [off]. *)
