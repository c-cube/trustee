...
let of_lex_pos ~ctx:_ctx p1 p2 : t =
  ...
  let off2 = max off1 (p2.pos_cnum - 1) in
  (* offset is one past end of token *)
  mk_ off1 off2
...
let pp ~ctx out (self : t) =
  if self == none then
    ...
    if l1 = l2 then
      Format.fprintf out "in file '%s', line %d columns %d..%d" ctx.filename l1
        c1 c2
    else
