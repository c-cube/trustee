...
let pp_sub out (s : sub) =
  Fmt.fprintf out "@[<v2>%s {@,%a%a@;<0 -2>}@]" s.sub_name
    Fmt.(list ~sep:(return "@ ") (pp_kv "import:"))
    s.imports
    Fmt.(some (pp_kv "@,package:"))
    s.package
      ...
      @@ tr []
           [
             ...
             td []
               [
                 a
                   [ A.href (spf "/art/%s" (H.Util.percent_encode art)) ]
                   [ txt art ];
               ];
           ]);
    ...
    div []
      [
        ...
        table'
          ...
          [
            sub_e @@ tr [] [ td [] [ txt "name" ]; td [] [ txt self.name ] ];
            sub_e
            @@ tr [] [ td [] [ txt "version" ]; td [] [ txt self.version ] ];
            sub_e
            @@ tr []
                 [
                   td [] [ txt "requires" ];
                              ...
                              li []
                                [
                                  a
                                    [
                                      A.href
                                        (spf "/thy/%s" (H.Util.percent_encode x));
                                    ]
                                    [ txt x ];
                                ])
                            self.requires);
...
let parse ~dir : t P.t =
  ...
  let rec pitem key =
    if CCString.suffix ~suf:":" key then (
      ...
      *> P.chars1_if (function
           | '\n' -> false
           | _ -> true)
      >>= fun v -> P.return (I_kv (key, v))
    ) else
      P.skip_space *> P.char '{' *> P.skip_white *> pitems [] >>= fun l ->
      P.skip_white *> P.return (I_sub (key, l))
  and pitems acc =
    P.skip_white
    *> (P.eoi *> P.return (List.rev acc)
       <|> P.char '}' *> P.return (List.rev acc)
       <|> ( pkey >>= fun key ->
             pitem key >>= fun i -> pitems (i :: acc) )
       <?> "expected items")
  in
  pitems [] >>= fun items ->
  let find ~msg l f =
  ...
  try
    ...
    and version =
      find items
        (function
          | I_kv ("version", s) -> Some s
          | _ -> None)
        ~msg:(fun () -> "no `version` provided")
    and requires =
      CCList.filter_map
        (function
          | I_kv ("requires", s) -> Some s
          | _ -> None)
        items
    in
    let meta =
        ...
        (function
          ...
          | I_sub (sub_name, l) ->
            let imports =
              CCList.filter_map
                (function
                  | I_kv ("import", s) -> Some (Util.unquote_str s)
                  | _ -> None)
                l
            in
            let package =
              CCList.find_map
                (function
                  | I_kv ("package", s) -> Some s
                  | _ -> None)
                l
            and article =
              CCList.find_map
                (function
                  | I_kv ("article", s) -> Some (Util.unquote_str s)
                  | _ -> None)
                l
            in
            let interp =
              CCList.flat_map
                (function
                  | I_kv ("interpret", s) ->
                    (match Interp_file.item_of_string s with
                    ...
                    let content =
                      ...
                      with _ ->
                        Trustee_core.Error.failf (fun k ->
                            k "cannot read interp. file '%s'" path)
                    in
                    (match Interp_file.of_string content with
                    | Ok l -> l
                    | Error err ->
                      Trustee_core.Error.(
                        raise
                        @@ wrapf "trying to read interp. file '%s'" path err))
                  | _ -> [])
                l
            in
