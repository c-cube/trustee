...
module Input = struct
  ...
  let of_chan ic =
    let iter_lines yield = iter_g yield (CCIO.read_lines_gen ic) in
    { iter_lines }
...
let pp_stack out self =
  ...
  let trail =
    if _r = [] then
      ...
      [ O_name (Name.of_string (Printf.sprintf "…(%d)" (List.length _r))) ]
  in
  Fmt.fprintf out "[@[<hv>%a@]]"
    Fmt.(list ~sep:(return ";@ ") pp_obj)
    (l @ trail)

let pp_vm out (self : t) : unit =
  Fmt.fprintf out "{@[stack:@ %a;@ dict={@[<hv>%a@]}@]}" pp_stack self
    (Fmt.iter Fmt.Dump.(pair int pp_obj))
    (CCHashtbl.to_iter self.dict)

let pp out self : unit =
  Fmt.fprintf out "{@[vm:@ %a;@ art: %a@]}" pp_vm self Article.pp (article self)

let pp_stats out self : unit =
  Fmt.fprintf out
    "(@[vm.stats@ :n-cuts %d :n-absThm %d@ :n-appThm %d@ :n-subst %d@])"
...
let version : rule =
 fun _ self ->
  ...
  | O_int n :: _ ->
    Error.failf (fun k -> k "expected version to be '5' or '6', not %d" n)
  | _ -> Error.failf (fun k -> k "version: expected an integer")
...
let absTerm : rule =
 fun _ self ->
  ...
  | _ -> Error.failf (fun k -> k "cannot apply absTerm@ in state %a" pp_vm self)
...
let absThm : rule =
 fun _ self ->
  ...
  | _ -> Error.failf (fun k -> k "cannot apply absThm@ in state %a" pp_vm self)
...
let mk_defined_ty_ c =
  ...
  | K.Const.C_arity n ->
    fun ctx _tyargs ->
      ...
      K.Expr.const ctx c _tyargs
  | K.Const.C_ty_vars _ ->
    Error.failf (fun k ->
        k "not a type const: `@[%a : %a@]`" K.Const.pp c K.Expr.pp
          (K.Const.ty c))
  ...
  try
    List.iter
      (fun th ->
        match
          if is_ty then
            K.Theory.find_ty_const th name
          else
            K.Theory.find_const th name
        with
        | None -> ()
...
let typeOp : rule =
 fun theory self ->
        ...
        fun ctx -> (function
          | [] ->
            let ind =
              ...
              | Some i -> i
              | None ->
                let ind = K.Theory.assume_ty_const theory "ind" ~arity:0 in
                (* special type *)
                self.ind <- Some ind;
                ind
            in
      ...
      | _ ->
        (try snd @@ Hashtbl.find self.named_tys n
         with Not_found ->
           ...
           let c =
             match lookup_const_in_scope_ self ~is_ty:true key with
             | Some c -> c
             | None ->
               Error.failf (fun k -> k "typeOp: unknown typeOp `%a`" Name.pp n)
           in
  ...
  | _ -> Error.failf (fun k -> k "cannot apply typeOp@ in state %a" pp_vm self)
...
let def : rule =
 fun _ self ->
  ...
  | _ -> Error.failf (fun k -> k "cannot apply def@ in state %a" pp_vm self)
...
let varType : rule =
 fun _ self ->
  ...
  | _ -> Error.failf (fun k -> k "cannot apply varType@ in state %a" pp_vm self)
...
let cons : rule =
 fun _ self ->
  match self.stack with
  | O_list l :: o :: st -> self.stack <- O_list (o :: l) :: st
  | _ -> Error.failf (fun k -> k "cannot apply cons@ in state %a" pp_vm self)
 ...
 fun _ self ->
  ...
  | O_list l :: O_ty_op (_, op) :: st ->
    let args =
      ...
      |> List.map (function
           | O_ty ty -> ty
           | o -> Error.failf (fun k -> k "typeOp: %a is not a type" pp_obj o))
    in
...
let var : rule =
 fun _ self ->
  ...
  | _ -> Error.failf (fun k -> k "cannot apply var@ in state %a" pp_vm self)

(* create a defined constant, with local type inference since OT
   gives us only the expected type of the constant *)
let mk_defined_const_ c =
  Log.debugf 3 (fun k ->
      k "mk defined const %a@ :args %a" K.Const.pp c K.Const.pp_args
        (K.Const.args c));
  match K.Const.args c with
  | K.Const.C_arity _ ->
    Error.failf (fun k -> k "not a term const: %a" K.Const.pp c)
  | K.Const.C_ty_vars [] ->
    (* non-polymorphic constant *)
    fun ctx _ty ->
  ...
  | K.Const.C_ty_vars ty_vars ->
    (* make new variables *)
    let vars =
      List.mapi (fun i v -> K.Var.makef "√%d" (K.Var.ty v) i) ty_vars
    in
    fun ctx ty ->
...
let const : rule =
 fun theory self ->
    ...
    let f_op =
      ...
      | { Name.path = []; name = "=" } ->
        fun ctx ty ->
          ...
          | _ -> Error.fail "= has an arrow type")
      | { Name.path = []; name = "select" } ->
        fun ctx ty ->
          (match K.Expr.view ty with
          | K.Expr.E_arrow (_, a) -> K.Expr.select ctx a
          | _ -> Error.fail "select has an arrow type")
      | _ ->
        (match Hashtbl.find self.named_consts n with
        | _, c -> c
        | exception Not_found ->
          (* lookup in external theories *)
          let key = Name.to_string n in
          let c =
            match lookup_const_in_scope_ self ~is_ty:false key with
            | Some c -> c
            | None ->
              Error.failf (fun k -> k "const: unknown constant `%a`" Name.pp n)
          in
          K.Theory.add_assumption_const theory c;
  ...
  | _ -> Error.failf (fun k -> k "cannot apply const@ in state %a" pp_vm self)
 ...
 fun _ self ->
  ...
  | O_int i :: st ->
    (try self.stack <- Hashtbl.find self.dict i :: st
     with Not_found -> Error.failf (fun k -> k "undefined ref %d" i))
  | _ -> Error.failf (fun k -> k "cannot apply ref@ in state %a" pp_vm self)
...
let constTerm : rule =
 fun _ self ->
  ...
  | O_ty ty :: O_const (_, c) :: st ->
    ...
    self.stack <- O_term t :: st
  | _ ->
    Error.failf (fun k -> k "cannot apply constTerm@ in state %a" pp_vm self)
...
let varTerm : rule =
 fun _ self ->
  ...
  | O_var v :: st ->
    ...
    self.stack <- O_term t :: st
  | _ -> Error.failf (fun k -> k "cannot apply varTerm@ in state %a" pp_vm self)
...
let defineConst : rule =
 fun theory self ->
  ...
  | _ ->
    Error.failf (fun k -> k "cannot apply defineConst@ in state %a" pp_vm self)

let defineConstList : rule =
 fun theory self ->
  match self.stack with
  | O_thm th :: O_list l :: st ->
    ...
    let names =
      List.map
        (function
          | O_list [ O_name n; O_var v ] -> n, v
          | _ -> Error.failf (fun k -> k "expected list of (name,var)"))
        l
    in
    ...
    if not (K.Var.Set.subset fvars_concl vars) then (
      ...
      Error.failf (fun k ->
          k
            "defineConstList: some free vars are not in hypothesis@ \
             :fvars-concl %a@ :vars %a"
            Fmt.(Dump.list K.Var.pp)
            (K.Var.Set.to_list fvars_concl)
            Fmt.(Dump.list K.Var.pp)
            (K.Var.Set.to_list vars))
    );
    ...
    let hyps_as_vars =
      List.map
        (fun hyp ->
          match K.Expr.unfold_eq hyp with
          | Some (v, rhs) ->
            (match K.Expr.view v with
            | K.Expr.E_var v -> v, rhs
            | _ -> Error.fail "expected hypothesis to have variable as LHS")
          | _ -> Error.fail "expected hypothesis to be an equation")
        hyps
    in
      ...
      CCList.fold_map
        (fun subst (n, v) ->
          let rhs =
            ...
            with Not_found ->
              Error.failf (fun k ->
                  k "cannot find hypothesis with var `%a`" K.Var.pp v)
          in

          let th, c = define_named_ self.ctx n rhs in
          self.art <-
            { self.art with Article.consts = c :: self.art.Article.consts };
          K.Theory.add_const theory c;
    ...
    let th = List.fold_left (fun th th' -> K.Thm.cut self.ctx th' th) th thms in
    Log.debugf 10 (fun k -> k "(@[defineConstList.result@ %a@])" K.Thm.pp th);

    self.stack <-
      O_thm th :: O_list (List.map (fun c -> O_const c) consts) :: st
  | _ ->
    Error.failf (fun k -> k "cannot apply defineConst@ in state %a" pp_vm self)
...
let pop : rule =
 fun _ self ->
  match self.stack with
  | _ :: st -> self.stack <- st
  | [] -> Error.failf (fun k -> k "cannot apply pop@ in state %a" pp_vm self)
 ...
 fun _ self ->
  ...
  | O_int i :: st ->
    let o =
      ...
      with Not_found -> Error.failf (fun k -> k "key %d not defined" i)
    in
    Hashtbl.remove self.dict i;
    self.stack <- o :: st
  | _ -> Error.failf (fun k -> k "cannot apply pop@ in state %a" pp_vm self)
...
let thm : rule =
 fun theory self ->
  ...
  | O_term _ :: O_list _ :: O_thm th :: st ->
    self.stack <- st;
    (* note: we skip the alpha renaming because of DB indices *)
    K.Theory.add_theorem theory th;
    self.art <-
      { self.art with Article.theorems = th :: self.art.Article.theorems }
  | _ -> Error.failf (fun k -> k "cannot apply thm@ in state %a" pp_vm self)
...
let refl : rule =
 fun _ self ->
  ...
  | O_term t :: st ->
    ...
    self.stack <- O_thm th :: st
  | _ -> Error.failf (fun k -> k "cannot apply refl@ in state %a" pp_vm self)
...
let betaConv : rule =
 fun _ self ->
  ...
  | O_term t :: st ->
    ...
    self.stack <- O_thm th :: st
  | _ ->
    Error.failf (fun k -> k "cannot apply betaConv@ in state %a" pp_vm self)
...
let axiom : rule =
 fun theory self ->
  ...
  | O_term t :: O_list _ :: st ->
    ...
    self.stack <- O_thm th :: st
  | _ -> Error.failf (fun k -> k "cannot apply axiom@ in state %a" pp_vm self)
...
let appThm : rule =
 fun _ self ->
  ...
  | O_thm a :: O_thm f :: st ->
    ...
    self.stack <- O_thm th :: st
  | _ -> Error.failf (fun k -> k "cannot apply appThm@ in state %a" pp_vm self)
...
let eqMp : rule =
 fun _ self ->
  ...
  | O_thm a :: O_thm f :: st ->
    ...
    self.stack <- O_thm th :: st
  | _ -> Error.failf (fun k -> k "cannot apply eqMp@ in state %a" pp_vm self)
...
let sym : rule =
 fun _ self ->
  ...
  | O_thm th :: st ->
    ...
    self.stack <- O_thm th :: st
  | _ -> Error.failf (fun k -> k "cannot apply sym@ in state %a" pp_vm self)
    ...
    let subst =
      List.fold_left
        (fun subst p ->
          ...
          | _ -> Error.failf (fun k -> k "expect first list to be a type subst"))
        K.Subst.empty tys
    in
    let subst =
      List.fold_left
        (fun subst p ->
          ...
          | O_list [ O_var v; O_term e ] ->
            Log.debugf 50 (fun k ->
                k "(@[vm.subst@ v=%a;@ t=`%a`;@ ty(t)=`%a`;@ same-ty=%B@])"
                  K.Var.pp_with_ty v K.Expr.pp e K.Expr.pp (K.Expr.ty_exn e)
                  (K.Expr.equal (K.Var.ty v) (K.Expr.ty_exn e)));
            K.Subst.bind subst v e
          | _ ->
            Error.failf (fun k -> k "expect second list to be an expr subst"))
        subst terms
    in
    let th = K.Thm.subst ~recursive:false self.ctx th subst in
    self.n_subst <- 1 + self.n_subst;
    self.stack <- O_thm th :: st
  | _ -> Error.failf (fun k -> k "cannot apply subst@ in state %a" pp_vm self)
...
let assume : rule =
 fun _ self ->
  ...
  | O_term e :: st ->
    ...
    self.stack <- O_thm th :: st
  | _ -> Error.failf (fun k -> k "cannot apply assume@ in state %a" pp_vm self)

let deductAntisym : rule =
 fun _ self ->
  match self.stack with
  ...
  | _ ->
    Error.failf (fun k ->
        k "cannot apply deductAntisym@ in state %a" pp_vm self)
...
let trans : rule =
 fun _ self ->
  ...
  | O_thm th1 :: O_thm th2 :: st ->
    ...
    self.stack <- O_thm th :: st
  | _ -> Error.failf (fun k -> k "cannot apply trans@ in state %a" pp_vm self)
...
let proveHyp : rule =
 fun _ self ->
  ...
  | O_thm th2 :: O_thm th1 :: st ->
    ...
    self.stack <- O_thm th :: st
  | _ ->
    Error.failf (fun k -> k "cannot apply proveHyp@ in state %a" pp_vm self)

let defineTypeOp : rule =
 fun theory self ->
  match self.stack with
  | O_thm th :: O_list names :: O_name rep :: O_name abs :: O_name tau :: st ->
    ...
    let ty_vars =
      List.map
        (function
          | O_name { path = []; name } ->
            K.Var.make name (K.Expr.type_ self.ctx)
          | _ -> Error.failf (fun k -> k "expect a list of names"))
        names
    in
    let def =
      K.Thm.new_basic_type_definition self.ctx ~ty_vars
    ...
    self.art <-
      { self.art with Article.consts = def.c_abs :: self.art.Article.consts };
    Hashtbl.add self.named_consts abs c_abs;

    K.Theory.add_const theory def.c_repr;
    let c_rep = rep, mk_defined_const_ def.c_repr in
    self.art <-
      { self.art with Article.consts = def.c_repr :: self.art.Article.consts };
    Hashtbl.add self.named_consts rep c_rep;

    K.Theory.add_const theory def.tau;
    let c_tau = tau, mk_defined_ty_ def.tau in
    self.art <-
      { self.art with Article.consts = def.tau :: self.art.Article.consts };
    Hashtbl.add self.named_tys tau c_tau;

    (* need to abstract over the theorems *)
    let repr_thm =
      let th = K.Thm.sym self.ctx def.repr_thm in
      (* flip first *)
      K.Thm.abs self.ctx def.repr_x th
    in
    let abs_thm = K.Thm.abs self.ctx def.abs_x def.abs_thm in

    self.stack <-
      O_thm repr_thm :: O_thm abs_thm :: O_const c_rep :: O_const c_abs
      :: O_ty_op c_tau :: st
  | _ ->
    Error.failf (fun k -> k "cannot apply defineTypeOp@ in state %a" pp_vm self)

let hdTl : rule =
 fun _ self ->
  match self.stack with
  | O_list (x :: tl) :: st -> self.stack <- O_list tl :: x :: st
  | _ -> Error.failf (fun k -> k "cannot apply hdTl@ in state %a" pp_vm self)
...
let mk_progress () =
  ...
  fun s ~line ->
    ...
    incr i;
    Fmt.printf "\x1b[2K\r[%c line %d] %s%!" cs.[n] line s;
    (* erase line; print current rule *)
    ()
  ...
  let self =
    {
      ctx;
      stack = [];
      dict = Hashtbl.create 32;
      named_consts = Hashtbl.create 32;
...
let parse_and_check_art_exn ~name (self : t) (input : input) :
  ...
  let process_line (theory : K.Theory.t) (s : string) : unit =
    ...
    if s = "" then Error.failf (fun k -> k "empty line (at line %d)" !line_);

    Option.iter (fun f -> f s ~line:!line_) self.progress_fun;
    ...
    | '0' .. '9' | '-' ->
      let n =
        try int_of_string s
        with _ -> Error.failf (fun k -> k "invalid integer at line %d" !line_)
      in
      self.stack <- O_int n :: self.stack
    | '"' ->
      let n = String.length s in
      if s.[n - 1] <> '"' then
        Error.failf (fun k -> k "expected closing \" at line %d" !line_);
      let s = String.sub s 1 (n - 2) in
      let n = Name.of_string s in
      self.stack <- O_name n :: self.stack
    | _ ->
      (match Str_map.find s rules with
      | r -> r theory self
      | exception Not_found ->
...
let parse_and_check_art ~name self i =
  try Ok (parse_and_check_art_exn ~name self i) with Error.E e -> Error e
