...
(** A storage wrapper that records all (key, bytes) stored into it. Create one
    before building the ctx; use [tracked_as_storage] to get the storage to pass
    to [K.Ctx.create]; then pass [ts] to [build]. *)
...
(** Load and decode one theory from the zip on demand. Caches result. Proof
    terms are NOT decoded; call [load_proofs] for that. *)

val load_proofs :
  zip_handle -> ctx:K.ctx -> string -> K.Linear_proof.t list option
(** Load proof terms for a previously-loaded theory, if available. Returns
    [None] if the entry has no proof section or the theory has not been loaded
    via [load_theory]. *)
...
(** Decode the expression at a minidag byte offset inside the named entry. Fast
    (cache hit) if the entry is already loaded. *)
...
val build :
  ...
  names:string Iter.t ->
  unit
(** Build a zip file from all theories. [ts] must be the tracked storage used
    when creating the ctx. [eval] must be the eval state that evaluated all the
    theories. *)
...
val lookup_theory :
  cache:string Str_tbl.t -> ctx:K.ctx -> string -> LP.t list option
