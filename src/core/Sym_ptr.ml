...
let equal a b =
  Component.equal a.name b.name &&
  CCList.equal Component.equal a.path b.path
...
module Map = struct
  ...
  let find_sub_exn k self : _ =
    let rec loop ks self = match ks with
  ...
  let namespace s self : _ t =
    mk_ None @@ M.singleton (Component.Str s) self
  ...
  let to_iter (self:_ t) k =
    ...
    M.iter (fun c sub ->
        iter_sub sub c
          (fun ptr sub -> match sub.value with
             | None -> ()
             | Some v -> k (ptr,v))
      ) self.sub
end
