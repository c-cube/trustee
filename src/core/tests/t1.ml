...
type clause = { lits: expr list } [@@deriving show { with_path = false }, eq]

let rec enc_expr enc (e : expr) : CB.cbor =
  CB.Enc.(
    match e with
    | Bool b -> add_entry enc @@ list [ text "b"; bool b ]
    | Var s -> add_entry enc @@ list [ text "v"; text s ]
    | App (f, a) ->
...
let enc_clause enc (c : clause) =
  CB.Enc.(
    add_entry enc @@ map [ text "lits", list (List.map (enc_expr enc) c.lits) ])
...
let cb1 =
  CB.Enc.(
    let enc = init () in
    let key = enc_clause enc c1 in
    finalize enc ~key)

let () = Format.printf "encoded c1: %a@." pp_cbor_pack cb1

let c1' =
  match CB.Dec.run dec_clause cb1 with
  | Ok c -> c
  | Error s -> failwith s
...
let c1'' =
  match CB.decode_string dec_clause c1_str with
  | Ok c -> c
  | Error e -> failwith e
