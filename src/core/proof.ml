...
module Linear_proof = struct
  ...
  let enc_arg enc =
    Cbor_pack.Enc.(
      ...
      | Pr_subst s ->
        let s =
          list
            (List.map (fun (v, e) -> pair (Expr.Util_enc_.enc_var enc v) (Expr.enc enc e)) s)
        in
        list [ text "s"; s ])
  ...
  let dec_arg ctx =
    Cbor_pack.Dec.(
      ...
      | [ `Text "e"; e ] ->
        ...
        Pr_expr e
      | [ `Text "s"; s ] ->
        let+ s = apply (list (pair (Expr.Util_dec_.dec_var ctx) (Expr.dec ctx))) s in
        Pr_subst s
      | _ -> fail "expected arg")
  ...
  let enc_step enc (self : step) =
    ...
    @@ map
         [
           text "concl", Expr.Util_enc_.enc_seq enc concl;
  ...
  let dec_step ctx =
    Cbor_pack.Dec.(
      let+ concl = field "concl" (Expr.Util_dec_.dec_seq ctx)
