...
and signature = S_app of node * node
...
module Sig_tbl = CCHashtbl.Make (struct
  ...
  let equal s1 s2 =
    match s1, s2 with
    | S_app (x1, y1), S_app (x2, y2) -> x1 == x2 && y1 == y2
...
let[@inline] res_thm_eq_ (th : K.thm) : E.t * E.t =
  ...
  | None ->
    Error.failf (fun k ->
        k "theorem %a should be an equation" K.Thm.pp_quoted th)
...
(* nodes are equal? *)
let[@inline] are_eq (n1 : node) (n2 : node) : bool = find n1 == find n2
...
let find_common_ancestor (self : t) (n1 : node) (n2 : node) : node =
  ...
  let res = find n2 in
  E.Tbl.clear tbl;
  (* be sure to reset temporary table *)
  res
...
let rec add_ (self : t) (e : E.t) : node =
  try E.Tbl.find self.nodes e with Not_found -> add_uncached_ self e
...
and add_uncached_ self e =
  ...
  if Option.is_some sigt then Vec.push self.to_update_sig @@ Update_sig node;
  node
...
let add_thm_ (self : t) (th : K.thm) : bool =
  ...
  | Some (a, b) ->
    ...
    if not (are_eq a b) then Vec.push self.to_merge (Merge (a, b, Exp_merge th));
    true
  | None -> false
...
let add_thm self th : unit =
  ...
  if not ok then
    Error.failf (fun k ->
        k "cannot add non-equational theorem %a" K.Thm.pp_quoted th)

let prove_cc_eqn (ctx : K.ctx) (hyps : K.thm list) (t : E.t) (u : E.t) :
    _ option =
...
let prove_cc_bool (ctx : K.ctx) (hyps : K.thm list) (concl : E.t) : _ option =
  ...
  | None ->
    (match
       List.partition
         (fun u -> Option.is_none (E.unfold_eq (K.Thm.concl u)))
         hyps
     with
    | [ hyp_p ], hyp_eqns ->
      (match prove_cc_eqn ctx hyp_eqns (K.Thm.concl hyp_p) concl with
      | None -> None
      | Some cc_lemma ->
    ...
    | _ ->
      Log.debugf 5 (fun k ->
          k "prove_cc_bool: cannot pick a predicate hypothesis");
      None)

let prove_cc_false (ctx : K.ctx) ~prove_false ~not_e (hyps : K.thm list) :
    _ option =
  let as_not e =
    match E.unfold_app e with
  ...
  let pos, neg =
    hyps
    |> CCList.partition_filter_map (fun th ->
  ...
  |> Iter.find_map (fun ((n_pos, th_pos), (_concl_neg, n_neg, th_neg)) ->
