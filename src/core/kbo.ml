...
module KBO_ = struct
  ...
  let create params : state =
    { wb=0; pos_counter = 0; neg_counter = 0;
      balance = E.Tbl.create 8; params;
    }
  ...
  let rec balance_weight st (e:E.t) (s:_ option) ~pos : bool =
    begin match E.view e with
      ...
      | E.E_const (c,_) ->
        ...
        false
      | E.E_var v ->
        if pos then incr_wb_ st else decr_wb_ st; (* as if it were a constant of weight 1 *)
        if not st.params.var_as_cst then (
          if pos then add_pos_var st e else add_neg_var st e;
        );
        (match s with Some s -> K.Var.equal s v | None -> false)
      | E.E_app (f, a) ->
        let r1 = balance_weight st f s ~pos in
        let r2 = balance_weight st a s ~pos in
        r1 || r2
      | E.E_lam (_,_ty,bod) ->
        if pos then incr_wb_ st else decr_wb_ st;
  ...
  let rec tc_kbo st (t1:E.t) (t2:E.t) : result =
    if E.equal t1 t2 then Eq
      ...
      | E.E_var x, _ ->
        ...
        if contains then Lt else Incomparable

      | _, E.E_var x ->
        add_neg_var st t2;
        decr_wb_ st;
        let contains = balance_weight st t1 (Some x) ~pos:true in
        if contains then Gt else Incomparable
      ...
      | _ ->
        ...
        let cmp_p = match E.view f1, E.view f2 with
          | E.E_const (c1,_), E.E_const (c2,_) ->
            (* FIXME: if params.var_as_cst then also do variable cases *)
            let cmp = st.params.precedence (K.Const.name c1) (K.Const.name c2) in
            if cmp<0 then Lt
            else if cmp=0 then Eq
            else Gt
          | _ -> Incomparable
        ...
        else if st.wb < 0 then l_or_none
        else if cmp_p = Gt then g_or_none
        else if cmp_p = Lt then l_or_none
  ...
  and tc_kbo_lex st (ts1:E.t list) (ts2:E.t list) : result =
    ...
    | _, [] ->
      ...
      Gt
    | t1 :: ts1, t2 :: ts2 ->
      begin match tc_kbo st t1 t2 with
