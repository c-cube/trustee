...
and expr = {
  ...
  mutable e_id: int;
  mutable e_flags: int;
      (* contains: [higher DB var | 1:has free vars | 5:ctx uid] *)
...
let expr_pp_with_ ~max_depth out (e : expr) : unit =
  let rec loop k ~depth names out e =
    ...
    | E_const (c, args) ->
      Fmt.fprintf out "(@[%a@ %a@])" Fmt.string (Cname.name c.c_name)
        (pp_list pp') args
...
module Expr_hashcons = Hashcons.Make (struct
  ...
  let equal a b =
    ...
    | E_kind, E_kind | E_type, E_type -> true
  ...
  let hash e : int =
    ...
    | E_type -> 12
    | E_const (c, l) ->
      H.combine4 20 (Cname.hash c.c_name) (expr_hash c.c_ty)
        (H.list expr_hash l)
...
module Util_chash_ = struct
  ...
  let rec hash_expr_ (e : expr) : H.t =
    let compute_ e =
      ...
      | E_const (c, args) ->
        H.string ctx "c";
        Cname.chasher ctx c.c_name;
        (* need to use name+hash *)
...
module Const = struct
  ...
  let expr_is_concrete_ (e : expr) : bool =
    try
      Expr0.iter_dag ~iter_ty:true e ~f:(fun e ->
          match e.e_view with
  ...
  let get_def (self : ctx) (c : t) : const_def option =
    ...
    Cname_LRU.get self.ctx_def_cache cname ~compute:(fun _ ->
  ...
  let get_def_exn self c =
    ...
    | Some d -> d
    | None ->
      Error.failf (fun k -> k "cannot find definition for %a" Cname.pp c.c_name)
end
...
module Expr = struct
  ...
  let is_eq_to_bool e =
    match view e with
    | E_const (c, []) -> Cname.equal c.c_name Const.cname_bool
    | _ -> false
    ...
    let ty =
      ...
      | C_arity n ->
        if List.length args <> n then
          Error.failf (fun k ->
              k "constant %a requires %d arguments, but is applied to %d"
                Cname.pp_name c.c_name n (List.length args));
        Lazy.from_val (Some c.c_ty)
      | C_ty_vars ty_vars ->
        if List.length args <> List.length ty_vars then
          Error.failf (fun k ->
              k "constant %a requires %d arguments, but is applied to %d"
                Cname.pp_name c.c_name (List.length ty_vars) (List.length args));
        lazy
  ...
  let[@inline] unfold_eq e =
    ...
    | E_const ({ c_name; _ }, [ _ ]), [ a; b ]
      when Cname.equal c_name Const.cname_eq ->
      Some (a, b)
    | _ -> None
...
module Linear_proof = struct
  ...
  let of_thm_proof (th0 : thm) : t =
    ...
    let rec traverse seqt pr : int =
      ...
      | None ->
        let i =
          match pr with
          | Proof.Pr_main pr' when seqt == th0.th_seq -> emit_step seqt pr'
          | _ -> emit_step seqt pr
        in
  ...
  let enc_arg enc =
    Cbor_pack.Enc.(
      ...
      | Pr_subst s ->
        let s =
          list
            (List.map (fun (v, e) -> pair (Var.enc enc v) (Expr.enc enc e)) s)
        in
        list [ text "s"; s ])
  ...
  let dec_arg ctx =
    Cbor_pack.Dec.(
      ...
      match l with
      | [ `Text "e"; e ] ->
        let+ e = apply (Expr.dec ctx) e in
        Pr_expr e
      | [ `Text "s"; s ] ->
        let+ s = apply (list (pair (Var.dec ctx) (Expr.dec ctx))) s in
        Pr_subst s
      | _ -> fail "expected arg")
...
module Theory = struct
  ...
  let add_assumption_const self (c : const) : unit =
    ...
    if Name_k_map.mem (kind, Cname.name c.c_name) self.theory_in_constants then
      Error.failf (fun k ->
          k "Theory.assume_const: constant `%a` already exists" Cname.pp_name
            c.c_name);
  ...
  let assume_ty_const self name ~arity : const =
    let c =
      Expr.new_ty_const self.theory_ctx name arity
  ...
  module Instantiate_ = struct
        ...
        | exception Not_found ->
          let u =
            ...
            | E_const (c, args) ->
              ...
              if Const.equal c c' && List.for_all2 Expr.equal args args' then
                e
              else
    ...
    and inst_const_ (self : state) (c : const) : const =
      ...
      | Some c' ->
        ...
        let chash =
          if Const.is_concrete_def def then
            Some c'.c_name.chash
          else
            None
        in
  ...
  let compose ?(interp = Str_map.empty) l th : t =
    ...
    if CCList.is_empty l then
      ...
      List.iter
        (fun th0 ->
          Name_k_map.iter
            (fun _ c -> Str_ty_tbl.replace const_tbl_ (c.c_name.name, c.c_ty) c)
      ...
      th.theory_in_constants <-
        Name_k_map.filter
          (fun _ c -> not (Str_ty_tbl.mem const_tbl_ (c.c_name.name, c.c_ty)))
          th.theory_in_constants;
