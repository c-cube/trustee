  ...
  let run args =
    ...
    List.iter
      (fun file ->
        ...
        | Ok str ->
          (match
             TS.Parser.parse_string_exn str TS.Parser.top ~filename:file
               ~notation
           with
          | l ->
        ...
        | Error e -> Error.failf (fun k -> k "cannot read '%s': %s" file e))
      args;
    ()
end
      ...
      List.iter
        (fun file ->
          CCIO.with_in file (fun ic ->
              ...
              match
                VM.parse_and_check_art ~name:(Filename.basename file) vm input
              with
              | Ok (_, art) ->
                Fmt.printf "; parsed and validated '%s'@." file;
                if !cat_ then Fmt.printf "%a@." Article.pp art;
                if not (VM.has_empty_stack vm) then (
                  Fmt.eprintf "VM stack is not empty@.";
                  exit 1
                );
...
let () =
  ...
  let help =
    Fmt.asprintf "@[<v>trustee_tk <cmd> [arg]*.@,@,Available commands:@ %a@]@."
      (pp_list (fun out (a, _) -> Fmt.fprintf out "@[<2>- %a@]" Fmt.text a))
      cmds
  in
  ...
  match !r with
  | None ->
    Fmt.eprintf "please provide a command@.";
    exit 1
  | Some r ->
