  ...
  let exec ~count (str : string) : _ Lwt.t =
    ...
    | eff_l ->
      ...
      let has_err =
        List.exists
          (function
            | VM.Eval_effect.Eff_print_error _ -> true
            | _ -> false)
          eff_l
      in
      let msg =
        if has_err then
          ...
          None
      in
      let res = { JK.Client.Kernel.msg; actions = [ mime ] } in
      Lwt.return (Ok res)
    ...
    | exception e -> Lwt.return_error (Printexc.to_string e)

  let kernel =
    JK.Client.Kernel.make ~codemirror_mode:"javascript" ~file_extension:"trsc"
      ~language_version:[ 0; 1 ] ~language:"trustee_script" ~exec ()
end
