...
module Make (E : EXPR) : S with module E = E = struct
  module E = E
  ...
  let pp (notation : Notation.t) out (e : E.t) : unit =
    ...
    *)
    let rec pp_rec_ (p : Fixity.precedence) k names out e : unit =
      ...
      let pp' p' out e = wrap_ p p' out @@ fun p -> pp_rec_ p k names out e in
      match E.view e with
        ...
        | Const (c, _), _ :: _ ->
          ...
          | Some (Fixity.F_binder p'), [ arg ] ->
            ...
            | Lambda_db (None, ty, bod) ->
              wrap_ p p' out @@ fun p ->
              Fmt.fprintf out "(@[%a x_%d:@[%a@].@ %a@])" Fmt.string c k pp_ty
                ty
                (pp_rec_ p (k + 1) (None :: names))
                bod
            | Lambda_db (Some v, ty, bod) ->
              wrap_ p p' out @@ fun p ->
              Fmt.fprintf out "(@[%a %s:@[%a@].@ %a@])" Fmt.string c v pp_ty ty
                (pp_rec_ p (k + 1) (Some v :: names))
                bod
            | _ -> default ())
          | Some (Fixity.F_left_assoc p'), [ a; b ] ->
            ...
            Fmt.fprintf out "@[%a@ %a %a@]"
              (pp' (p + 1))
              a Fmt.string c (pp' p) b
          | Some (Fixity.F_right_assoc p'), [ a; b ] ->
            wrap_ p p' out @@ fun p ->
            Fmt.fprintf out "@[%a@ %a %a@]" (pp' p) a Fmt.string c
              (pp' (p + 1))
              b
      ...
      | Lambda_db (None, ty, bod) ->
        Fmt.fprintf out "(@[\\x_%d:@[%a@].@ %a@])" k pp_ty ty
          (pp_rec_ 0 (k + 1) (None :: names))
          bod
      | Lambda_db (Some v, ty, bod) ->
        Fmt.fprintf out "(@[\\%a:@[%a@].@ %a@])" Fmt.string v pp_ty ty
          (pp_rec_ 0 (k + 1) (Some v :: names))
          bod
      | Box ([], c) -> Fmt.fprintf out "@[|- %a@]" (pp_rec_ 0 k names) c
      | Box (hyps, c) ->
        Fmt.fprintf out "@[<hv>%a@ |- %a@]"
          (pp_list @@ pp_rec_ 0 k names)
          hyps (pp_rec_ 0 k names) c
...
module Pp_k_expr : S with type E.t = Trustee_core.Kernel.Expr.t = Make (struct
  ...
  let view e =
    ...
    | E.E_box seq -> Box (K.Sequent.hyps_l seq, K.Sequent.concl seq)
