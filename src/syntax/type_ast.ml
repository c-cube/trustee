  ...
  let as_queryable ~ty_as_q (self : _ t) =
    object
      ...
      method pp out () =
        ...
        Fmt.fprintf out "@[bound variable:@ %a@]" (pp_with_ty Queryable.pp) self
    end
end
...
module Expr = struct
  ...
  let[@inline] is_eq_to_type (self : t) =
  ...
  let rec pp out (e : t) : unit =
    ...
    | Lambda _ ->
      ...
      Fmt.fprintf out "(@[lambda (@[%a@])@ %a@])" (pp_list pp_bvar_ty) vs pp bod
    | Eq (a, b) -> Fmt.fprintf out "(@[=@ %a@ %a@])" pp a pp b
    | Meta m -> Meta_var.pp out m
    | Let (bs, bod) ->
      Fmt.fprintf out "(@[let@ (@[%a@])@ %a@])" (pp_list pp_binding) bs pp bod
  ...
  let iter ?(rec_ty = true) ~f ~f_bind b_acc (e : expr) : unit =
    ...
    match view e with
    | Kind | Type | Const { args = []; _ } | Meta _ | Var _ | BVar _ | Error _
      ->
      ()
...
module Top = struct
  ...
  *)
  ...
  let pp out (self : t) : unit =
    ...
    | Def { name; vars = []; ret; body } ->
      Fmt.fprintf out "(@[<1>def %a ()@ %a@ %a@])" Const.pp name Expr.pp ret
        Expr.pp body
    | Def { name; vars; ret; body } ->
      Fmt.fprintf out "(@[<1>def %a (@[%a@]) %a@ %a@])" Const.pp name
        (pp_list Expr.pp_var_ty) vars Expr.pp ret Expr.pp body
    | Decl { name; ty } ->
      Fmt.fprintf out "(@[<1>decl %a@ %a@])" Fmt.string name Expr.pp ty
    | Fixity { name; fixity } ->
  ...
  *)
  ...
  let iter_errors f (self : t) : unit =
    iter_ self ~f_expr:(Expr.iter_errors f) ~f_goal:ignore (* TODO *)
      ~f_proof:ignore (* TODO *)
  ...
  let error_set_l (l : t list) : Error_set.t =
    object
      method iter_errors f = List.iter (iter_errors f) l
    end
  ...
  let as_queryable (self : t) : Queryable.t =
    object
      ...
      method! children yield =
        ...
        iter_ self
          ~f_expr:(map_yield Expr.as_queryable)
          ~f_goal:ignore (* TODO *)
          ~f_proof:ignore
      (* TODO *)
