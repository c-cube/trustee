...
module State : sig
  ...
  let create ~env () : t = { to_gen = []; env; i = 0; fvars = Str_map.empty }
end
  ...
  module Expr = struct
    ...
    let subst_bvars (m : t ID.Map.t) (e : t) : t =
      let rec aux m e =
        ...
        | Some ty ->
          ...
          | Error err -> mk_ ~loc ~ty @@ Error err)
      ...
      and rename_bvar m (v : bvar) : _ * bvar =
        ...
        let v' = { v with id = ID.copy v.id; ty } in
        let v'_as_e = mk_ ~loc:v.loc ~ty:v'.ty @@ BVar v' in
    ...
    module Unif : sig
      ...
      let pp_err_trace out (st : err_trace) : unit =
        ...
        List.iter
          (fun (e1, e2) ->
            Fmt.fprintf out "@[<2>- unifying@ %a@ and %a@];@ " pp (deref_ e1) pp
              (deref_ e2))
          st;
        Fmt.fprintf out "@]"
      ...
      let contains_meta ~sub_m (e : expr) : bool =
        let rec aux () e =
          match view e with
          | Meta m' when TA.Meta_var.equal sub_m m' -> raise_notrace E_contains
          | _ -> iter () e ~f_bind:(fun () _ -> ()) ~f:aux
        in
        try
          aux () e;
          false
      ...
      let contains_bvar ~(bv : bvar) (e : expr) : bool =
        let rec aux () e =
          match view e with
          | BVar v when ID.equal v.id bv.id -> raise_notrace E_contains
          | _ -> iter () e ~f_bind:(fun () _ -> ()) ~f:aux
        in
        try
          aux () e;
      ...
      and unif_rec_ (a : expr) (b : expr) : unit =
        let[@inline] fail_ st a b = raise_notrace (E ((a, b) :: st)) in
        let rec aux st renaming a b =
          ...
          if a == b then
            ...
            | BVar a, BVar b when ID.equal a.id b.id || same_bvar_ renaming a b
              ->
              ()
            ...
            | Eq (a1, a2), Eq (b1, b2) | App (a1, a2), App (b1, b2) ->
              aux st' renaming a1 b1;
              aux st' renaming a2 b2
            | Meta m1, Meta m2 when TA.Meta_var.equal m1 m2 -> ()
            | Meta m1, _ ->
              if contains_meta ~sub_m:m1 b then fail_ st a b;
              assert (m1.deref == None);
              m1.deref <- Some b
            | _, Meta m2 ->
              if contains_meta ~sub_m:m2 a then fail_ st a b;
              assert (m2.deref == None);
              m2.deref <- Some a
            | Let _, _ -> fail_ st a b (* TODO? *)
            | ( ( Type | Kind | Var _ | BVar _ | Eq _ | Error _ | Const _
                | App _ | Ty_arrow _ | Lambda _ ),
                _ ) ->
              fail_ st a b
          )
    ...
    let ty_arrow_l ~loc args ret : ty =
    ...
    let const ~loc (c : TA.const) args : t =
      ...
      let@ () =
        Error.guard
          (Error.wrapf ~loc "expr.const %a %a" TA.Const.pp c (Fmt.Dump.list pp)
             args)
      in
      match c.args, args with
      ...
      | C_vars vars, _ ->
        if not (List.length args = List.length vars) then
          Error.fail "wrong arity";
        if not (List.for_all is_ty args) then
          Error.fail "an argument is not a type";

        let subst =
          List.fold_left2
            (fun m (v : bvar) a -> ID.Map.add v.id a m)
            ID.Map.empty vars args
        in
    ...
    let app f (arg : t) : t =
      ...
      let ty =
        match Unif.ty_app ~loc f arg with
        | Ok ty -> ty
        | Error err ->
          Error.failf ~loc (fun k ->
              k "Cannot apply `%a`@ to `%a`@ trace: %a" pp f pp arg
                Unif.pp_err_trace err)
      in
      mk_ ~loc ~ty @@ App (f, arg)

    let app_l (f : t) (args : t list) : t = List.fold_left app f args
    ...
    let unif_exn_ ~loc e a b =
      ...
      | Ok () -> ()
      | Error st ->
        Or_error.failf ~loc (fun k ->
            k
              "@[<hv2>type error@ @[while inferring the type @[<2>of %a@]@]:@ \
    ...
    let unif_type_with_ ~loc e ty =
      ...
      | Error st ->
        Or_error.failf ~loc (fun k ->
            k
              "@[<hv2>type error@ @[<2>while unifying the type @[<2>of %a@]@ \
               with %a@]:@ unification error in the following trace:@ %a@]"
    ...
    let rec infer_ ~reify (bv : binding Str_map.t) (e : A.Expr.t) : expr =
      ...
      | AE.Var v when Str_map.mem v.name bv ->
        ...
        (match Str_map.find v.name bv with
      ...
      | AE.Var va ->
        ...
        | None, None ->
          ...
          var ~loc v)
      | AE.Wildcard -> State.new_ty_meta state ~loc
      | AE.Meta _ ->
        assert false
      ...
      | AE.With (vs, bod) ->
        let bv =
          List.fold_left
            (fun bv (v : AE.var) ->
              let ty = infer_ty_opt_ ~reify ~loc ~default:type_ bv v.ty in
      ...
      | AE.Bind { b; vars; body } ->
        ...
        let b =
          ...
          | None ->
            Error.failf ~loc (fun k -> k "Unknown constant %a" A.Const.pp b)
          | Some c -> c
        in
        ...
        CCList.fold_right
          (fun bv body ->
            ...
            let e = app b lam in
            Log.debugf 5 (fun k ->
                k "app: f=%a@ ty=%a" Expr.pp e Expr.pp (ty_exn e));
            e)
          vars body
    ...
    and infer_const_ ~loc (c : TA.Const.t) : expr =
      ...
      in
      let args = CCList.init arity (fun _ -> State.new_ty_meta ~loc state) in
      const ~loc c args
    ...
    let infer_reify (e : A.Expr.t) : expr = infer_ ~reify:true Str_map.empty e
  end

  module Top = struct
    let infer_ ~reify (top : A.Top.t) : TA.Top.t list =
      let loc = A.Top.loc top in
      match A.Top.view top with
      | A.Top.Show e ->
