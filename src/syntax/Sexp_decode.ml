      ...
      (fun s ->
        ...
        | Some l ->
          let get u =
            ...
            | Error e -> raise (E e)
          in
          (try Ok (List.map get l) with E e -> Error e)
        | None ->
          let msg () =
            match what with
            | None -> spf "expected %s" what_l
            | Some w -> spf "expected %s of %s" what_l w
          in
          fail_ msg s);
  }
...
let sub_l p l =
  ...
  in
  { run = (fun _ -> try Ok (List.map p1 l) with E err -> Error err) }

let try_catch p : _ t = { run = (fun s -> Ok (p.run s)) }

let pair a b =
  let* l = list_of value in
  match l with
  | [ x; y ] ->
    let+ x = sub a x and+ y = sub b y in
    x, y
...
let tuple3 a b c =
  ...
  match l with
  | [ x; y; z ] ->
    let+ x = sub a x and+ y = sub b y and+ z = sub c z in
    x, y, z
