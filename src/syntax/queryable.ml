(** Queryable types for the LSP server.
    ...
    A queryable type is one that the user can query in various ways, the primary
    one being "hover". Other ways are LSP classics such as "go to definition",
    "infer type", etc. *)
...
class virtual t =
  object
    ...
    (** Print the object in a user-readable way. This is useful for hover
        queries. *)

    method def_loc : Loc.t option = None
    (** Location of the definition of this object, if any. This is useful for
        go-to-def queries. *)

    method defining : (string * Loc.t) Iter.t = Iter.empty
    (** Symbols defined/bound in this object, if any. This is useful for listing
        symbols in a side-view or hierarchy. *)
  end
...
(** Build a queryable object from a value, along with a printer and a location.
*)
let mk_pp ?def_loc ?(defining = Iter.empty) ~loc ~pp x : t =
  object
    inherit t
    method loc = loc
