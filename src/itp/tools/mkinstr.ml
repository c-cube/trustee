...
let instrs : (string * op_arg list * doc) list =
  [
    ...
    ( "call",
      [ Int ],
      "(<n values> chunk -- ) Pop and call the chunk or primitive that is on \
       top of the stack with the top <n> values below it" );
    ( "ret",
      [],
      "( -- ) Return values from current chunk (number determined by chunk \
       arity)" );
    ...
    ( "extract",
      [ Int ],
      "(vs -- vs vs[-i]) extract <i>-th value, where 0 is top of the stack.\n\
       `extract 0` is `dup`." );
    "rstore", [ Int ], "(x -- ) Pop value and store it into register <i>";
    ( "rload",
      [ Int ],
      "( -- x) Load value from register <i> and push it onto stack" );
    ( "lload",
      [ Int ],
      "( -- x) Load i-th local value of current chunk and push it onto stack" );
    ...
    ( "elam",
      [],
      "(var expr -- expr) Pops variable `v` and body `e`, and pushes `\\v. e`."
    );
    ( "econst",
      [],
      "(c []ty -- expr) Pops constant and type arguments, pushes expression." );
    "econst0", [], "(c -- expr) Pops nullary constant, pushes expression.";
    ( "econst1",
      [],
      "(c ty -- expr) Pops unary constant and parameter, pushes expression." );
    ( "deapp",
      [],
      "(expr -- expr? expr? bool) Pops expression, returns `f a true` if it's \
       `f a`, pushes `nil nil false` otherwise." );
    ( "delam",
      [],
      "(expr -- var? expr? bool) Pops expression, returns `v bod true` if it's \
       `\\v.bod`, `nil nil false` otherwise." );
    ( "devar",
      [],
      "(expr -- var? bool) Pops expression, returns `v true` if it's variable \
       `v`, `nil false` otherwise." );
    ( "deconst",
      [],
      "(expr -- const? []ty? bool) Pops expression, returns `c args true` if \
       it's `c` applied to arguments `args`; returns `nil nil false` \
       otherwise.." );
    ...
    ( "thabs",
      [],
      {|(th var -- th) Pops `|- t=u` and `v`, pushes `|- \v.t=\v.u`.|} );
    ( "thcongr",
      [],
      "(th th -- th) Pops `|- f=g` and `|- a=b`, pushes `|- f a=g b`." );
    ...
    ( "thbeta",
      [],
      {|(expr -- th) Pops `(\x. t) u`, pushes `|- (\x.t) u = t[x:=u]`.|} );
    ( "thsubst",
      [],
      "(th subst -- th) Instantiate the theorem with the substitution." );
    ( "dth",
      [],
      "(th -- []expr expr) Pops `F |- e`, pushes array `F` and conclusion `e`."
    );
