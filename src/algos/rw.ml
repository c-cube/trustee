...
let thm_res_eqn thm : E.t * E.t = unfold_eqn_ (K.Thm.concl thm)
...
let bottom_up_apply conv ctx e = Conv.apply (bottom_up conv) ctx e
...
let under_apply p conv ctx e = Conv.apply (under p conv) ctx e
...
module Rule = struct
  ...
  and rhs =
    | Basic of { th: K.thm }
    | Dynamic of { mk_rhs: K.ctx -> K.expr -> K.Subst.t -> K.thm option }
  ...
  let pp out (self : t) =
    match self.rhs with
    | Basic { th; _ } -> Fmt.fprintf out "(@[rw.rule@ %a@])" K.Thm.pp_quoted th
    | Dynamic _ ->
      Fmt.fprintf out "(@[rw.dyn-rule@ :lhs %a@])" K.Expr.pp self.lhs

  let to_conv (self : t) : conv =
   fun ctx e ->
    ...
    | Some subst ->
      ...
      | Basic { th } ->
        ...
        Rw_step th2
      | Dynamic { mk_rhs } ->
        (match mk_rhs ctx self.lhs subst with
        | None -> Same
        | Some th2 ->
          assert (E.equal e (thm_res_lhs th2));
          Rw_step th2))
...
module AC_rule = struct
  ...
  let make ctx ~f ~assoc ~comm () : t =
    ...
    and comm_shape = app_eq (app_l f [ x; y ]) (app_l f [ y; x ]) in

    if not (Unif.is_alpha_equiv assoc_shape (Th.concl assoc)) then
      Error.failf (fun k ->
          k "wrong shape for associativity theorem:@ expected `%a`@ got %a" E.pp
            assoc_shape E.pp (Th.concl assoc));
    if not (Unif.is_alpha_equiv comm_shape (Th.concl comm)) then
      Error.failf (fun k ->
          k "wrong shape for commutativity theorem:@ expected `%a`@ got %a" E.pp
            comm_shape E.pp (Th.concl comm));

    let rule_assoc = Rule.mk_rule assoc in
    let rule_comm = Rule.mk_rule comm in

    let rw_with rule e : Th.t = Conv.apply (Rule.to_conv rule) ctx e in
    let rw_at pos rule e : Th.t = under_apply pos (Rule.to_conv rule) ctx e in
    ...
    *)
    ...
    and r2 =
      ...
      *)
      let r1 =
        rw_at Pos.(eqn1 @@ app0 @@ app1 @@ root) rule_comm (Th.concl assoc')
      in
