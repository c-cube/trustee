(** {1 Rewriting}
    ...
    This module contains rewriting structures and algorithms that use the
    {!Conv} interface. *)
...
(** Bottom-up rewriting. The [conv] is called on every subterm, starting from
    the leaves. *)
...
module Rule : sig
  type t
  (** A rewrite rule, with a LHS (left-hand side) pattern and some way of
      computing the RHS (right-hand side) from it when it matches a term. *)
  ...
  val mk_rule : K.thm -> t
  (** A rewrite rule, i.e. an equation. The theorem should be [|- lhs = rhs] to
      rewrite instances of [lhs] into instances of [rhs]. *)
  ...
  (** [mk_dynamic lhs f] can generate a rule [|- subst(lhs) = rhs] on the fly,
      or [None] *)
...
module RuleSet : sig
  ...
  val to_conv : t -> conv
  (** Converter that tries each rule in the rule set. The implementation might
      be more efficient that trying each rule one by one, so the order in which
      rules are tried is {b not} specified. *)
end
