...
impl RawMessage {
    pub(crate) fn read_poll(
        ...
        timeout_ms: i64,
    ) -> Result<Option<RawMessage>, Error> {
        let n = connection
            .socket
            .poll(zmq::PollEvents::POLLIN, timeout_ms)?;
        if n == 0 {
            return Ok(None);
        }
        ...
        if let Some(mac_template) = &connection.mac {
            ...
            if let Err(error) = mac.verify(&hex::decode(&hmac)?) {
                return Err(anyhow!("hmac check failed: {}", error));
            }
        }
...
impl JupyterMessage {
    pub(crate) fn read_poll(
        connection: &mut Connection,
        timeout_ms: i64,
    ) -> Result<Option<JupyterMessage>, Error> {
        let raw_message = match RawMessage::read_poll(connection, timeout_ms)? {
            None => return Ok(None),
            Some(m) => m,
        };
    ...
    pub(crate) fn read(connection: &mut Connection) -> Result<JupyterMessage, Error> {
        loop {
            match Self::read_poll(connection, 200)? {
                None => (),
                Some(m) => return Ok(m),
            }
        }
    ...
    pub(crate) fn cursor_pos(&self) -> usize {
        self.content["cursor_pos"].as_usize().unwrap_or(0)
    }
